...
var commentContainer = {
	...
	C.addComment = function() {
		...
		if(C.commentList[id] == undefined) {
			...
			// c.style, c.loc, c.content, c.loc. 
			c.id = id;
			$("#post-content").append('<div class="bullet" id="' + x + '" style="left:' + C.size.width + 'px;top:0px;">' + '<div style="' + c.style + '">' + c.content + '</div></div>');
	...
	C.refresh = function() {
		...
		$('.flyingbox').each(function(){
			...
			this.style.left=C.size.width;
			if(parseFloat(this.left) + parseFloat(this.offsetWidth) > 0) {
			    C.commentList[this.id].speed *= 20;
			} else {
			...
			if(C.commentList[this.id].row != -1) {
			    C.commentList[this.id].speed = (commentList[this.id].len + C.size.width) / C.duration;
			    C.rowStatus[row].tailSpeed = speed;
			    C.rowStatus[row].tailLoc = C.size.width;
			}
		    }
		    if(parseFloat(this.left) + parseFloat(this.offsetWidth) > 0) {
			this.left = (parseFloat(this.left) - commentList[this.id].speed) + 'px';
			this.top = (parseFloat(bp.size.top) + 30 * commentList[this.id].row) + 'px';
		    }
		});
		for(i = 0; i < maxRow; i++) {
